...
def vtm_encoding(preprocessed_yuv_name, bitstream_name, compress_log_name, wdt, hgt, qp, bit_depth):
    ...
    # subp.run(f"../vtm_coding/EncoderAppStatic -c ../vtm_coding/encoder_intra_vtm.cfg -i {preprocessed_yuv_name} -o \"\" -b {bitstream_name} -q {qp} --ConformanceWindowMode=1 -wdt {wdt} -hgt {hgt} -f 1 -fr 1 --InternalBitDepth={bit_depth} --InputBitDepth={bit_depth} --InputChromaFormat=400 --OutputBitDepth={bit_depth}",
...
def vtm_pipeline(org_feat_path, vtm_root_path, quantization_mapping_name, model_type, trun_flag, samples, max_v, min_v, trun_high, trun_low, quant_type, bit_depth, QP):
    # Set related paths
    ...
    for idx, feat_name in enumerate(feat_names):
        ...
        # elif quant_type == 'density' or quant_type == 'kmeans':
        #     quantization_mapping = load_quantization_points(quantization_mapping_name)
        #     dequant_feat = nonlinear_dequantization(unpack_feat, quantization_mapping) 
...
if __name__ == "__main__":
    ...
    trun_flag = False; samples = 10; bit_depth = 10
    if trun_flag == False: trun_high = max_v; trun_low = min_v

    quant_type_all = ['uniform', 'kmeans']
    QPs = [0]
        ...
        for QP in QPs:
            ...
            if task == 'dpt':
                ...
                for ch in range(len(trun_high)):
                    quant_mapping_name = f'/home/gaocs/projects/FCM-LM/Code/vtm_coding/quantization_mapping/{task}/quantization_mapping_{task}_ch{ch}_trunl{trun_low[ch]}_trunh{trun_high[ch]}_{quant_type}_{samples}_bitdepth{bit_depth}.json'
            ...
            vtm_pipeline(org_feat_path, vtm_root_path, quantization_mapping_name, model_type, trun_flag, samples, max_v, min_v, trun_high, trun_low, quant_type, bit_depth, QP)