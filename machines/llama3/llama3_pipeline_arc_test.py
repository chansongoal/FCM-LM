...
class ARCChallengeSolver:
    ...
    def calculate_coding_averages(self):
        with open('/home/gaocs/projects/FCM-LM/Code/llama3/log.txt', 'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            if len(parts) >= 3:  
                try:
                    eval_enc_time, eval_dec_time, bits_str = parts[0], parts[1], parts[2].strip('[]')
                    bits = int(bits_str) if bits_str.isdigit() else int(eval(bits_str))
                    total_enc_time += float(eval_enc_time)
                    total_dec_time += float(eval_dec_time)
                    total_bits += bits
                except ValueError:
                    print("Data parsing error! Skip current line!")

        num_lines = len(lines)  
        if num_lines > 0:  
            avg_enc_time = total_enc_time / num_lines
            avg_dec_time = total_dec_time / num_lines
            avg_bits = total_bits / num_lines
        else:
            avg_enc_time, avg_dec_time, avg_bits = 0, 0, 0  

        print("avg_enc_time: ", avg_enc_time)
        print("avg_dec_time: ", avg_dec_time)
    ...
    def run(self):
        ...
        # self.calculate_coding_averages()
...
if __name__ == "__main__":
    ...
    # org_json_path = "/home/gaocs/projects/FCM-LM/Data/llama3/csr/source/ARC-Challenge-Train.jsonl"
    org_json_path = "/home/gaocs/projects/FCM-LM/Data/llama3/csr/source/sampled_longest100.jsonl"
    result_json_path = f'/data/xuyenan/llama3/output/arc_c_results_{datetime.now().strftime("%Y%m%d%H%M%S%f")}.json'
    # sampled_longest100_ids = "/home/gaocs/projects/FCM-LM/Data/llama3/csr/source/sampled_longest100_ids.txt"
